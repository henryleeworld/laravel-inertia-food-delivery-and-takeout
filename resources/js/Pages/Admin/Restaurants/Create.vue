...
<template>
  <Head title="{{ $t('Add New Restaurant') }}" />

  <AuthenticatedLayout>
    <template #header>
      <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ $t('Add New Restaurant') }}</h2>
    </template>
            ...
            <div class="p-6 text-gray-900 overflow-x-scroll">
              <form @submit.prevent="submit" class="flex flex-col gap-4">
                <div class="form-group">
                  <InputLabel for="restaurant_name" :value="$t('Restaurant Name')" />
                  <TextInput
                    id="restaurant_name"
                    type="text"
                ...
                <div class="form-group">
                  <InputLabel for="email" :value="$t('Owner Email')" />
                  <TextInput
                    id="email"
                    type="email"
                    v-model="form.email"
                ...
                <div>
                  <PrimaryButton :disabled="form.processing">{{ $t('Create New Restaurant') }}</PrimaryButton>
                </div>
              </form>
            </div>
          </div>
