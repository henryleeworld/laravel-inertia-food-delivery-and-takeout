...
<template>
  <Head :title="$t('Add New Product')" />

  <AuthenticatedLayout>
    <template #header>
      <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ $t('Add New Product') }}</h2>
    </template>
          ...
          <div class="p-6 text-gray-900 overflow-x-scroll">
            <div class="p-6 text-gray-900 overflow-x-scroll">
              <form @submit.prevent="submit" class="flex flex-col gap-4">
                ...
                <div>
                  <PrimaryButton :processing="form.processing"> {{ $t('Create New Product') }} </PrimaryButton>
                </div>
              </form>
            </div>
          </div>
